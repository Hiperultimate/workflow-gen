...
authRoutes.post('/signup', async (req, res) => { 
    ...
    const checkRecord = await prisma.user.findUnique({ where: { email: enteredCreds.email } });
    if (checkRecord) {
    ...
    try {
        ...
        return res.status(200).json({ message: "User registered succesfully!", email : createUser });
...
authRoutes.post('/signin', async (req, res) => { 
    ...
    if (!checkUserRecord) {
      return res.status(400).json({ message: "User not found, please enter valid details" });
    }

    const isPasswordValid = await compare(enteredCreds.password, checkUserRecord.password);
    if(!isPasswordValid) return res
      .status(400)
      .json({ message: "Invalid password" });

    const auth_token = jwt.sign(checkUserRecord, env.JWT_KEY, { expiresIn : 60 * 60 * 24 });
